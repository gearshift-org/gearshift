...
A Claude Chat is a pane kind (`kind: "chat"`) inside the same split-pane tab type as terminals, so splitting, layout, focus, maximize, and closing are shared; the pane's id is the chat id. Splitting copies the kind of the pane being split, and chat panes never start a PTY. Chats otherwise use a separate path from terminal panes. The renderer sends prompts through the preload bridge to the Electron main process, where the Claude Agent SDK launches the system `claude` binary using the login-shell PATH. A short-lived SDK query calls `supportedModels()` to populate the chat model menu and its model-specific effort levels without sending a prompt. SDK stream events become chat text and tool activity over IPC; `canUseTool` requests wait for an approve/deny reply from the chat; several can be pending at once, each shown and answered separately. Edit, MultiEdit, and Write calls carry their before/after text so the chat can show diffs. "Allow for this session" returns the request's permission `suggestions` as `updatedPermissions`, with every destination forced to `session`. The main process keeps each pending prompt and the turn's status, so a chat that reloads mid-turn asks for them and resumes showing the live turn. Replies are stored as ordered parts (text runs and tool calls) so they render in the order Claude produced them. `AskUserQuestion` goes through the same hook: its questions are shown as a prompt, and the user's choices return as `answers` in the tool's `updatedInput`. Stop calls the SDK's `interrupt()`; if the turn hasn't ended after 1.5s the process is closed, and the turn is marked finished a second later regardless, so the chat can't get stuck. Each turn starts with the chat's selected permission mode (`auto` by default); changing it mid-turn calls `setPermissionMode` on the live query. Full access (`bypassPermissions`) also needs the SDK's `allowDangerouslySkipPermissions`, which is set only on turns that start in Full access, so a turn can't be switched into it midway. Pasted or dropped images are read in the renderer, scaled to at most 1568px on the long side when larger, and sent as base64 image content blocks ahead of the prompt text; the transcript keeps only small thumbnails. Each turn's prompt is sent as a streaming input: messages sent while the turn runs are pushed into it with `priority: "next"`, so Claude Code folds them into the running turn. After a turn's `result`, the input stays open briefly for a follow-up turn a late message may start, then the turn is reported finished. The process stays up a few more seconds for the `prompt_suggestion` Claude Code sends after a turn (enabled with `promptSuggestions`), which becomes the input placeholder; starting another turn in the chat ends it first; messages sent after that are queued in the renderer and sent as the next turn. Each turn stores Claude's session ID so the next turn can resume it. The empty-chat session picker lists the project's sessions with the SDK's `listSessions({ dir })` and loads one with `getSessionMessages()`, converting typed prompts and Claude's text and tool calls into chat messages; the chat then adopts that session ID. The main process sends the session ID to the chat as soon as Claude Code reports it, then checks the session title with the SDK's `getSessionInfo()` every 2.5 seconds while the turn runs, sending it whenever it changes (first the prompt, then Claude's generated title, which is written in the background). The renderer also checks once when the chat mounts and after each turn. The title becomes the tab name. The chat transcript and model/effort selections are stored by chat (pane) ID in the workspace state store. Closing a chat pane, or its tab, stops its active query and removes that transcript. PTY lifecycle hooks and terminal status polling apply only to terminal tabs. A chat pane instead reports its own `TerminalAgentStatus` (working during a turn, needs-attention while a permission or question waits, completed when a reply finishes while the pane isn't in view, cleared once it is), so the pane border, sidebar, tab bar, and project indicators treat it like an agent terminal. It never reports an agent name, so it isn't picked as the "last agent terminal" that actions type into. Its `lastSubmitAt` also stamps the pane's persisted `lastMessageAt` and the project's `updatedAt`, which is how chat messages move the tab and project in the sidebar's recent ordering (terminal prompts do the same through chat history).

Terminal panes detect supported coding agents by asking the Electron main process to inspect the PTY shell's child process tree. GearShift passes each terminal a `GEARSHIFT_SESSION_ID` and `GEARSHIFT_AGENT_SOCKET` so supported lifecycle hooks can report status back over a local Unix socket. Agent-specific hooks and plugins are normalized into GearShift lifecycle events: `start`, `stop`, `needs_attention`, (Claude and Grok) `subagent_start`/`subagent_stop`, and (Claude) `background_wait`. Claude's `Stop` hook fires whenever the main agent finishes a turn, even while background subagents are still running, so GearShift tracks subagent lifecycles and holds the "completed" notification until the last pending background subagent has stopped; Grok's parallel subagents get the same treatment.

A `Stop` hook only means Claude finished its reply, not that the work is over, so the renderer never shows "done" the moment `stop` arrives:

- **Settle window** — a `stop` that closes an active turn is held for `STOP_SETTLE_MS` (2.5s). Any other non-subagent hook event in that window (for example a `PostToolUse` `start` after another `Stop` hook blocked the stop and made Claude keep going) cancels it. When the window ends, pending subagents are checked again before the completion is sent.
- **Background shells** — on Claude's `Stop`, the hook script walks up from its parent process to the `claude` process and looks for child shells whose command line contains `shell-snapshots`. Foreground Bash commands have already exited by then, so any shell still running is a background Bash/Monitor task that will wake Claude up again when it finishes. The script then sends `background_wait` instead of `stop`, and the pane keeps working until the next real `stop`. Background tasks that never exit (dev servers, watchers) fall back to a completion after `BACKGROUND_WAIT_MAX_MS` (10 min) with no further hook activity.
- **Subagent stops** — a `Stop` payload that carries `agent_id` fired inside a subagent and is sent as `subagent_stop`. Subagent ids come from `subagent_id` first, then `agent_id` (Grok: `agentId`), and whitespace in the JSON is allowed. A `subagent_stop` with an id that isn't pending is ignored instead of clearing another subagent that is still running; only a stop with no id at all clears the oldest anonymous entry.
...
**Batching is only allowed where it cannot delay an echo.** Screens repaint at most once a frame, so flushing _more often_ than that buys no earlier pixels — it only adds work. What matters is that the bytes have arrived and been parsed before the frame comes:
...
Every open project keeps its whole workspace mounted — hidden via `hiddenLayerClass` (`content-visibility:hidden`, so no layout or paint) and with `isVisible=false` on its terminals, but mounted, because the panes hold live xterm state and agent status the sidebar shows. That makes anything that runs _per mounted pane_ scale with how many projects the user has open, which is the one dimension the user grows over a session. Two things did:
...
Project notes (no longer surfaced in the right sidebar; the storage and HTTP endpoint remain) are stored in the same local SQLite database as chat history (`gearshift.db`), in `project_notes` keyed by `project_id`. The renderer reads and saves notes through `window.term.notes`; note edits are debounced for 200 ms before writing to SQLite. The existing loopback-only history server exposes notes to local agents via `GET /notes?projectId=<id>` on the same port as `/history`. The endpoint returns Markdown with the project ID, available project metadata, and the current notes for that project.
...
File opens from the tree, command palette, or diff context menu reuse one full-workspace preview inside the active terminal tab. Git changes use the same preview surface. Opening a file or Git change temporarily hides the terminal layout without unmounting it, and closing the preview immediately restores the terminal with its state intact. The preview persists across reloads and Cmd/Ctrl+W closes it rather than its terminal tab. When no terminal exists, previews use the current empty workspace without adding a visible sidebar tab. Opening another file or Git change replaces the current preview instead of creating a sidebar tab. Local dev-server links clicked in terminals (`localhost`, `127.0.0.1`, `0.0.0.0`, or `[::1]`) open in one shared in-app dev preview tab, updating that tab when a different local URL is selected.
...
An idle agent terminal shows a top-center floating Git action using the same pill treatment as the terminal's scroll-to-bottom control. A dirty working tree shows `Commit and Push`; an otherwise clean branch with commits ahead of its upstream, or a branch without an upstream yet, shows `Push Changes`. The action is scoped to the active agent in that terminal, stays hidden while the agent is working or waiting for input, and closes when the current turn is canceled. Clicking it submits the matching instruction to that agent rather than running Git directly in the renderer.
...
`terminateSession` is the single teardown path used by explicit tab/pane close and daemon shutdown. node-pty calls `setsid`, so the shell's pid is the process-group leader; we signal the whole group with `process.kill(-pid, "SIGHUP")`, then `SIGKILL` as a fallback after a short grace if the session hasn't exited. If node-pty still doesn't emit `onExit`, the daemon destroys the internal stream and drops the session record so stale PTY master fds are released. **Never replace this with a bare `pty.kill()`** — that reintroduces the fd leak.

The daemon never expires a live terminal based on age or inactivity. Quiet shells and long-running processes stay alive until the user closes their pane/tab or the process exits on its own. This avoids destroying work merely because a terminal has not received input or output recently.
