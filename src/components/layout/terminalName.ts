...
export function hasAgentActivityTitleSignal(
  title: string | undefined
): boolean {
...
export function displayName(t: TerminalTab): string {
  ...
  const activePane = t.panes.find((p) => p.id === t.activePaneId) ?? t.panes[0]
  const paneCustomName = activePane?.customName?.trim()
  if (paneCustomName) return paneCustomName
  ...
  return (
    autoFromPane ||
    agentDisplayName(activePane?.agentStatus?.agentName) ||
    t.name
  )
}
...
export function paneDisplayName(pane: TerminalPane, index: number): string {
