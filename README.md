...
The tab title follows the Claude session's title, the same one Claude Code shows in `/resume`: a `/rename` title if set, otherwise Claude's generated title or your first prompt. It updates after each reply. Chat messages and Claude's session ID are saved with the workspace. Reopening the app restores the chat and resumes the same Claude conversation on the next message. Closing a chat pane, or its tab, removes its saved messages. Terminal-based Claude tabs remain available separately.
